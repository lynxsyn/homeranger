...
- **Fake seams.** AI/email/discovery providers sit behind env-gated fakes so unit/integration/E2E never hit a paid API: `RESEND_FAKE` `EXTRACTION_FAKE` `ANALYSIS_FAKE` `VISION_FAKE` `EMBEDDING_FAKE` `MATCH_FAKE` `DISCOVERY_FAKE` `OUTREACH_FAKE` (=`1`). The full worker fake set also needs `RESEND_FROM` + the R2 vars to boot.
