...
describe("savedListingRepository (real pgvector)", () => {
  it("saves idempotently and lists a user's saved listing ids", async () => {
    ...
    expect(await savedListingRepository.listSavedListingIds(USER_A)).toEqual([
      listingId,
    ]);
  });
  ...
  it("isolates saved listings across owners (incl. the operator NULL namespace)", async () => {
    ...
    expect(await savedListingRepository.listSavedListingIds(USER_B)).toEqual([]);
    expect(await savedListingRepository.listSavedListingIds(null)).toEqual([]);
