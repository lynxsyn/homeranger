...
// Single-user homeranger: one DATABASE_URL, one PrismaClient. No admin/RLS
// second connection (Doxus needed prismaAdmin for its BYPASSRLS admin role;
// homeranger is single-user with no RLS, so that split is intentionally gone).
// The local default matches the homeranger-postgres dev port (5434, the
