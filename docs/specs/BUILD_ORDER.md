...
> The core product loop is built. The rows below are the next genuine enhancements (deferred during the Scouts reframe), in priority order — pick one up when prioritised, or refresh from `docs/plans/homeranger-plan.md` + `docs/decisions/`.
